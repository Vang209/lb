    ...
    public TakeReturn TakeBook(Client client, Book clientBook){
        ...
        for (int i = 0; i < BookRepository.books.size(); i++) {
            if (clientBook.equals(BookRepository.books.get(i)) && BookRepository.books.get(i).getClientId() == null) {
                ...
                DatabaseRepository.UpdateLogs(new Log(BookRepository.books.get(i).getId(), client.getId(), "Взял", date)); //запись лога в базу
                DatabaseRepository.UpdateBook(BookRepository.books, BookRepository.books.get(i));  //обновление книги в базе
            }
        }
        if (numberOfBooks == 0) {
            return null;
    ...
    public TakeReturn ReturnBook(Client client, Book filingBookData){
        ...
        for (int i = 0; i < BookRepository.books.size(); i++) {
            if (clientBook.equals(BookRepository.books.get(i)) || client.getUuid().equals(BookRepository.books.get(i).getId())) {
                ...
                LocalDate date = LocalDate.now();
                DatabaseRepository.UpdateLogs(new Log(BookRepository.books.get(i).getId(), client.getId(), "Вернул", date)); //запись лога в базу
